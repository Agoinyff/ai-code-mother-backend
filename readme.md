...
## 上述过程之后我们已经可以实现对话历史功能，让AI能够基于历史上下文进行网站的迭代优化。之后我将开发让AI生成更复杂的前端工程化项目， 比如Vue 的工程文件
## 方案选型分析：
### 通过对一些大厂平台的调研，有以下三种方案：
### 1.直接输出Markdown，和之前的思路一样让AI输出结构化的Markdown代码片段，然后进行解析保存为文件，优点就是实现简单，好理解，实时展示效果好，但是缺点也很明显，如果代码量太大，一次对话可能无法完整输出
### 2.使用工具调用的方式：给AI提供保存文件的工具，让AI调用工具进行文件保存，优点是可以让AI生成更复杂的项目结构，缺点是实现复杂度高，实时性差
### 3.Agent模式：它以大语言模型未核心，集成记忆，知识库，工具调用等多种能力，能够让AI更智能地完成复杂任务。即先规划再执行。但是缺点就是整体架构更加复杂，需要自己设计Agent流程，需要多次调用AI，输出结果更加不可控成本也更高
###   由于langchain4j本身就支持AI多次调用工具，相当于已经实现了基础的Agent多步骤执行能力   经过分析，选择方案二，使用工具调用的方式
### 完整步骤：生成完Vue工程代码之后，是无法直接运行的需要执行 npm install 和 npm run build打包构建，会得到一个dist目录，网站流量和部署都应该是访问这个目录
### 通过Hutool的RuntimeUtil结合Java的ProcessBuilder实现执行外部命令行指令